...
export const seedEmployees = async () => {
  ...
  toast.info('Starting to import 26 employees...');
  ...
  for (const emp of employeesList) {
    ...
    if (error) {
      toast.error(`Failed to add ${emp.name}: ${error.message}`);
  ...
  toast.success(`Successfully imported ${added} employees!`);
  // Refresh page to load new employees
  if(added > 0) {
    setTimeout(() => {
